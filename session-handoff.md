...
> 최종 갱신: 2026-09-02 (Session 013 — 입력 보조 복원)
...
## ⚠ 먼저 읽을 것 — 방향이 바뀌었다 (2026-09-02)

**복사·붙여넣기 방식으로 새로 썼고, 전 구간이 실제로 돈다.**
`career.html` → `career-step1`(1차 프롬프트 복사) → `career-report1`(1차 결과 붙여넣기)
→ `career-step2`(추가정보 → 2차 프롬프트 복사) → `career-report2`(2차 결과 + 합본 저장).
프록시 방식 코드 8파일은 2026-09-02 에 폐기했다.

- 계획은 `feature_list.json` 의 **`paste-001`~`006`** (전부 `not_started`)
- 폐기한 기능은 **`retired`** 상태 + evidence 보존 — 다시 만들 때의 요구사항이 거기 있다
- 아래 "Verified Now" 의 진로상담 항목은 **폐기된 코드에 대한 과거 기록**이다.
  사실이지만 지금 코드에 대한 주장이 아니다 — 그 위에 쌓지 말 것
- **로그인 데모 · `.gs` · 구글시트는 그대로 살아 있다**

**1차 → 2차 → 합본까지 전 구간이 돈다.** `paste-001`·`002`·`007` 이 `passing` 이다.

⚠ **사람이 한 번 확인해야 닫히는 것**: 복사된 프롬프트를 실제 AI 화면에 붙여넣어
잘리지 않고 들어가는지. `clipboard.readText` 권한과 합성 `Ctrl+V` 가 모두 막혀
자동 검증이 안 된다 — 쓰기 성공과 글자 수까지만 확인했다.

`paste-003`(완결성 검사)까지 끝났다. 붙여넣을 때마다 1차 A~L·2차 STEP 1~11·
면책 문장 위치를 검사해 잘림을 잡는다. **기대 형식이 `career.js` 와
...
입력 보조도 `newPrjt01` 수준으로 복원했다(`paste-009`) — 빠른 선택(고르면 덧붙음, 중복 무시)·예시 채우기·학교급 연동.
⚠ 선택지 목록은 `newPrjt01` 것 그대로다. **새로 지어내지 말 것** — 특히 '일하는 방식' 9종은 2차 프롬프트가 열거한 항목과 같아야 한다. 희망 직업·직무는 자유입력으로 남긴다.
...
결정: **프록시는 저장소에 남기고, 진로상담 앱만 복사·붙여넣기로 간다.**
`careerTest` 가 같은 Apps Script 배포의 `doGet` 과 구글시트를 계속 쓰기 때문이다.

`init.ps1` 은 **느슨하게 만들지 않고 방향을 뒤집었다** —
"`fetch(endpoint)` 가 있는가"는 제거하고, "폐기 코드가 되살아나 있지 않은가",
"죽은 링크가 남아 있지 않은가", "프롬프트 원문이 살아 있는가"를 넣었다.
`.gs`·시트·`/exec` 유출·BOM·인코딩 검사는 대상이 살아 있으므로 그대로 뒀다.
검사 수는 73 → 48. **대상이 사라진 만큼 줄어든 것이다.**

> careerTest 는 같은 Apps Script 배포의 `doGet` 을 쓴다.
> **이 저장소에서 프록시를 지워도 careerTest 는 영향을 받지 않는다.**
...
| 프록시 버전 | **1.8.1** | 배포본도 1.8.1 이어야 한다 — `.\init.ps1 -Live` 로 확인 |
...
- 🟡 **재배포 대기 — 저장소=1.8.1 / 배포=1.7.2.**
  ⚠ **급하지 않다.** `paste-006` 에서 프록시를 제거하기로 하면 이 재배포는 불필요해진다.
  방향을 먼저 확정하고 판단할 것. (원래 내용은 아래 그대로 둔다)
- 🔴 **재배포 대기 상세 — 저장소=1.8.1 / 배포=1.7.2.** 공용 `토큰로그` 탭 미러링을 끈
