...
- **Current highest-priority unfinished feature**: **`paste-006`** — 프록시 경로 은퇴 여부 결정.
  이것이 선행 결정이고, 그 뒤 `paste-001`(프롬프트 복사) → `paste-002`(결과 붙여넣기) 순서다
- **Current blocker**: **`init.ps1` 이 새 방향을 막는다.** 171줄의 `fetch(endpoint)` 검사가
  *"프롬프트 복사 방식으로 되돌아가지 않았는지"* 를 본다 — 복사·붙여넣기로 바꾸는 순간 검증이 실패한다.
  프록시를 강제하는 검사가 12건 더 있다. `paste-006` 에서 함께 정리해야 한다
