...
> 최종 갱신: 2026-09-02 (Session 007 — newPrjt02 분리 · 분석 엔진 방향 전환 결정)
...
**분석 엔진을 프록시 자동 호출에서 프롬프트 복사·붙여넣기로 바꾼다.**
앱이 조립한 프롬프트를 사용자가 복사 → 원하는 AI 화면에 붙여넣기 → 받은 md 를
앱에 되돌려 넣기. API 키·프록시·요금·Apps Script 실행한도가 전부 사라진다.

- 계획은 `feature_list.json` 의 **`paste-001`~`006`** (전부 `not_started`)
- **아직 코드는 하나도 바뀌지 않았다.** 이 세션은 방향만 정해 문서에 넣었다
- 아래 "Verified Now" 는 **프록시 방식 기준의 과거 검증 결과**다. 사실이지만
  곧 은퇴할 경로에 대한 것이다 — 그대로 믿고 그 위에 쌓지 말 것

### 선행 결정 — `paste-006`

`init.ps1` 이 지금 **정반대를 강제한다.** 171줄이 `fetch(endpoint)` 를 찾아
*"프롬프트 복사 방식으로 되돌아가지 않았는지"* 를 검사한다. 프록시를 강제하는 검사가
12건 더 있다. 이걸 정리하지 않으면 새 방향으로 바꾸는 순간 `.\init.ps1` 이 실패한다.

**하네스를 느슨하게 만들라는 뜻이 아니다.** 결정이 바뀌었으니 검사도 새 방향을
집행하는 쪽으로 바꾸는 것이다 — 지우는 게 아니라 방향을 돌리는 일이다.
(`MIRROR_TO_SHARED` 를 껐을 때 검사를 "미러링이 있는가"에서 "꺼져 있는가"로
뒤집은 것과 같은 방식이다.)
